...
public class GlobalExceptionHandler {
	...
	public ResponseEntity<ValidationErrorResponse> handleBeanValidation(MethodArgumentNotValidException ex) {
		Map<String, String> errors = ex.getBindingResult()
			.getFieldErrors()
			.stream()
			.collect(Collectors.toMap(FieldError::getField, FieldError::getDefaultMessage, (a, b) -> a)); // keep
																											// first
		log.warn("Bean validation failed – {} field errors", errors.size());
		return ResponseEntity.badRequest()
			.body(new ValidationErrorResponse(HttpStatus.BAD_REQUEST.value(), "Validation error", LocalDateTime.now(),
					errors));
	}
	...
	public ResponseEntity<ErrorResponse> handleDataIntegrity(DataIntegrityViolationException ex) {
		...
		return body(HttpStatus.CONFLICT, "Resource conflict: a record with the same unique identifier already exists");
	}
	...
	private ResponseEntity<ErrorResponse> body(HttpStatus status, String msg) {
		return ResponseEntity.status(status).body(new ErrorResponse(status.value(), msg, LocalDateTime.now()));
	}
